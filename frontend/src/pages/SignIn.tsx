...
// Define API base URL for the login endpoint
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/login`, {
        ...
        // IMPORTANT: Must include credentials for session cookie transfer
        credentials: 'include', 
        body: JSON.stringify({
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          toast.error(
            language === 'pa'
              ? 'ਇਸ ਈਮੇਲ ਨਾਲ ਕੋਈ ਖਾਤਾ ਨਹੀਂ ਮਿਲਿਆ। ਕਿਰਪਾ ਕਰਕੇ ਪਹਿਲਾਂ ਸਾਈਨ ਅਪ ਕਰੋ।'
              : language === 'hi'
          ...
          toast.error(
            language === 'pa'
              ? 'ਗਲਤ ਪਾਸਵਰਡ। ਕਿਰਪਾ ਕਰਕੇ ਦੁਬਾਰਾ ਕੋਸ਼ਿਸ਼ ਕਰੋ।'
              : language === 'hi'
      ...
      <div className="w-full max-w-6xl">
        ...
        <div className="bg-card rounded-2xl shadow-xl overflow-hidden grid md:grid-cols-2">
          ...
          <div className="bg-gradient-to-br from-primary/90 to-accent/90 p-12 text-white flex flex-col justify-center">
            ...
            <h3 className="text-3xl font-bold mb-4 text-center">
              {language === 'pa' ? 'ਕਾਰੀਗਰ ਭਾਈਚਾਰੇ ਵਿੱਚ ਸ਼ਾਮਲ ਹੋਵੋ' : language === 'hi' ? 'कारीगर समुदाय में शामिल हों' : 'Join the Artisan Community'}
            </h3>
            <p className="text-white/90 text-center mb-8 text-lg">
              {language === 'pa' 
                ? 'ਸਾਥੀ ਕਾਰੀਗਰਾਂ ਨਾਲ ਜੁੜੋ, ਆਪਣਾ ਕੰਮ ਦਿਖਾਓ, ਅਤੇ AI-ਸੰਚਾਲਿਤ ਸਮੱਗਰੀ ਰਚਨਾ ਨਾਲ ਆਪਣੇ ਕਾਰੋਬਾਰ ਨੂੰ ਵਧਾਓ।'
